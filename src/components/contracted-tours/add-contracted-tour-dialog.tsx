...
import { PlusIcon, CalendarIcon, Trash2, MapPin } from "lucide-react";
...
export function AddContractedTourDialog() {
  ...
  const form = useForm<TourFormValues>({
    ...
    defaultValues: {
      ...
      exportCustoms: "",
      importCustoms: "",
      unloadingStops: [{ location: "", unloadingDate: null }],
      price: "",
      company: "",
      isADR: false,
    },
  ...
  const onSubmit = async (values: TourFormValues) => {
    setLoading(true);
    try {
      const response = await fetch("/api/contracted-tours", {
        method: "POST",
      ...
      if (!response.ok) {
        ...
        throw new Error(error.message || "Greška pri kreiranju ture");
      }

      toast.success("Ugovorena tura je uspješno kreirana");
      setOpen(false);
      form.reset();
      router.refresh();
      ...
      toast.error(
        error instanceof Error ? error.message : "Greška pri kreiranju ture"
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[900px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Nova ugovorena tura</DialogTitle>
          <DialogDescription>
            Unesite podatke o ugovorenoj turi
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
